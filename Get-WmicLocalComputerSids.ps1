<#
.SYNOPSIS
  The "Get-WmicLocalComputerSids" function is a WMIC command wrapper for getting a list of built-in SIDs and their corresponding human-readable names.

.DESCRIPTION
.EXAMPLE
  PS C:\> WmicLocalComputerSids | ft
  ...
  Here we run the function using its built-in alias of "WmicLocalComputerSids".  The output consists of local SIDs and their corresponding group or identity name.

.INPUTS
.OUTPUTS
.NOTES
  Name: Get-WmicLocalComputerSids.ps1
  Author: Travis Logue
  Version History: 1.2 | 2021-01-15 | Changed Name and Alias
  Dependencies:
  Notes:
  ...
  .
#>
function Get-WmicLocalComputerSids {
  [CmdletBinding()]
  [Alias('WmicLocalComputerSids')]
  param (
